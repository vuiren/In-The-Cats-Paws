...
namespace Game_Code.MonoBehaviours.Players
{
    public class PlayerSmartCat : Player
    {
        ...
        private INetworkFreeRoomPointsSync _freeRoomPointsSync;
        ...
        public void Construct(SceneData sceneData, INetworkTurnsSync networkTurnsSync,
            ...
            IUnitRoomService unitRoomService, IRoomsService roomsService, INetworkFreeRoomPointsSync freeRoomPointsSync)
        {
            _networkRoomsSync = networkRoomsSync;
            _unitRoomService = unitRoomService;
            _networkUnitsSync = networkUnitsSync;
            _roomsService = roomsService;
            _freeRoomPointsSync = freeRoomPointsSync;
        ...
        private void SelectUnit(Unit unit)
        {
            ...
            Logger.Log(this,$"Selecting unit: {unit.name}");
            SelectionService.SelectUnit(unit, this);
            CameraController.selectedUnit = unit.UnitGameObject();
            Logger.Log(this,$"Done selecting unit: {unit.name}");
        }

        private void DeselectUnit(IUnit unit)
        {
            Logger.Log(this,$"Deselecting unit: {unit.UnitGameObject().name}");
            SelectionService.DeselectUnit(unit, this);
            CameraController.selectedUnit = null;
            Logger.Log(this,$"Done deselecting unit: {unit.UnitGameObject().name}");
        }
        ...
        protected override void SpawnControllableUnits()
        {
            ...
            foreach (var spawnPoint in _unitsSpawnPoints)
            {
                ...
                var pointForUnit = _freeRoomPointsSync.GetFreePointFromRoom(spawnPoint.SpawnRoom);
        ...
        public override void MakeStep(Vector3 cursorPos)
        {
            ...
            if (room)
            {
                Logger.Log(this,$"Found room: {room.name}");
            }


            if (unit)
            {
                Logger.Log(this,$"Found unit: {unit.name}");
                SelectUnit(unit);
            }
            ...
            Logger.Log(this,$"Setting room for unit: {selectedUnit.UnitGameObject().name}");
        ...
        private void UnitGoToRoom(Room room)
        {
            ...
            _freeRoomPointsSync.AddFreePointToRoom(unitRoom, selectedUnit.UnitGameObject().transform.position);
            var pointForUnit = _freeRoomPointsSync.GetFreePointFromRoom(room);
            selectedUnit.SetTargetPointForUnit(pointForUnit);
        }
        
        private void UnitGoToRoom(Unit unit, Room room)
        {
